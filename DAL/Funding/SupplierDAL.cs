...
namespace DAL.Funding
{
    public class SupplierDAL : GenericService<Supplier>
    {
        ...
        public DataTable GetRoomBySupplierId(long supplier_id, string ServiceName,long HotelId, int page_index = 1, int page_size = 10)
        {
            try
            {
                SqlParameter[] objParam = new SqlParameter[5];
                objParam[0] = new SqlParameter("@SupplierId", supplier_id);
                objParam[1] = new SqlParameter("@PageIndex", page_index);
                objParam[2] = new SqlParameter("@PageSize", page_size);
                objParam[3] = new SqlParameter("@ServiceName", ServiceName);
                if(HotelId == 0)
                {
                    objParam[4] = new SqlParameter("@HotelId", DBNull.Value);
                }
                else
                {
                    objParam[4] = new SqlParameter("@HotelId", HotelId);
                }
