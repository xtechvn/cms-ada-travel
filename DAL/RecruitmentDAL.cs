...
namespace DAL
{
    public class RecruitmentDAL : GenericService<Recruitment>
    {
        ...
        public DataTable GetPagingList(ArticleSearchModel searchModel, int currentPage, int pageSize)
        {
            try
            {
                ...
                return _DbWorker.GetDataTable(StoreProcedureConstant.SP_GetlistRecruitment, objParam);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    using (var transaction = _DbContext.Database.BeginTransaction())
                    {
                        try
                        {
                            var article =  _DbContext.Set<Recruitment>().FirstOrDefault(s=>s.Id==Id);
                            ...
                            model.Categories = await _DbContext.Set<RecruitmentCategory>().Where(s => s.ArticleId == article.Id).Select(s => (int)s.CategoryId).ToListAsync();
                    ...
                    using (var transaction = _DbContext.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (ListCateId != null && ListCateId.Count > 0)
                            {
                                ...
                                {
                                    var model = new RecruitmentCategory
                                    {
                                        CategoryId = item,
                                        ArticleId = ArticleId
                                    };
