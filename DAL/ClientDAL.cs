...
namespace DAL
{
    public class ClientDAL : GenericService<Client>
    {
        ...
        public async Task<DataTable> GetListClientCustomerCareFund(CustomerManagerViewSearchModel searchModel)
        {
            try
            {
        ...
        public async Task<int> UpdateStatusClient(int status, int id)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    var model = _DbContext.Clients.FirstOrDefault(s => s.Id == id);
                    model.Status = status;
                    var Update = _DbContext.Clients.Update(model);
                    _DbContext.SaveChanges();
                    return 1;
        ...
        public async Task<List<Client>> GetClientByphone(string Phone)
        {
            try
            {
                using (var _DbContext = new EntityDataContext(_connection))
                {
                    return await _DbContext.Clients.AsNoTracking().Where(x => x.Phone == Phone).ToListAsync();
                }
