                    ...
                    case (int)EmailType.SaleDH:
                        {
                            ...
                            try
                            {
                                ...
                                if (order != null)
                                {
                                    ...
                                    var data_sql = await _userRepository.GetUserSuggesstion(null, list_user_ids.Where(x => x.Id != null).Select(x => (int)x.Id).ToList());
                                    if (order.OperatorId != null)
                                    {
                                        ...
                                        foreach (var item in cc_split)
                                        {
                                            ...
                                            //}
                                            if (TpDHid != 0 && data_sql.Where(S => S.Id == Convert.ToInt32(item)).ToList().Count > 0)
                                            {
                                                var TpDH = await _userRepository.GetById(Convert.ToInt32(TpDHid));
                                                if (TpDH != null) { message.To.Add(TpDH.Email); }
                                            }
            ...
            try
            {
                if (Emailmodel == null)
                {
                    ...
                    if (type != 1)
                    {
                        ...
                        body = body.Replace("{{MANCC}}", model.SupplierId.ToString());
