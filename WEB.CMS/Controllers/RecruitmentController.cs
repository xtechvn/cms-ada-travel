...
namespace WEB.CMS.Controllers
{
    ...
    {
        private const int NEWS_CATEGORY_ID = 64;
        ...
        public IActionResult Search(ArticleSearchModel searchModel, int currentPage = 1, int pageSize = 20)
        {
            ...
            try
            {
                model = _RecruitmentRepository.GetPagingList(searchModel, currentPage, pageSize);
        ...
        public async Task<IActionResult> Detail(long Id)
        {
            ...
            if (Id > 0)
            {
                model = await _RecruitmentRepository.GetArticleDetail(Id);
            }
            else
            {
        ...
        public async Task<string> GetSuggestionTag(string name)
        {
            try
            {
                var tagList = await _RecruitmentRepository.GetSuggestionTag(name);
        ...
        public async Task<IActionResult> AddOrUpdate(int id, int parent_id)
        {
            ...
            if (id > 0)
            {
                var article = await _RecruitmentRepository.GetArticleDetail(id);
        ...
        {
            var images = await _RecruitmentRepository.GetFanpageImagesAsync(articleId);
            return Json(images);
        }
        ...
        public async Task<IActionResult> SaveFanpageImages([FromBody] FanpageImageSaveModel model)
        {
            ...
            await _RecruitmentRepository.SaveFanpageImagesAsync(model.ArticleId, model.Images);
        ...
        public async Task<IActionResult> UpSert([FromBody] object data)
        {
            try
            {
                ...
                var articleId = await _RecruitmentRepository.SaveArticle(model);
        ...
        public async Task<IActionResult> ChangeArticleStatus(long Id, int articleStatus)
        {
            try
            {
                ...
                var rs = await _RecruitmentRepository.ChangeArticleStatus(Id, articleStatus);
        ...
        public async Task<IActionResult> DeleteArticle(long Id)
        {
            ...
            {
                var Categories = await _RecruitmentRepository.GetArticleCategoryIdList(Id);
                var rs = await _RecruitmentRepository.DeleteArticle(Id);
