    ...
    public partial class PricePolicyController : Controller
    {
        ...
        public async Task<IActionResult> PolicyRule(string campaign_id, string hotel_id,DateTime? from_date, DateTime? to_date)
        {
            ...
            try
            {
                ...
                HotelPricePolicyCampaignModel model = new HotelPricePolicyCampaignModel();

                if (hotelid > 0)
                {
                     model = await _campaignRepository.GetPolicyDetailViewByCampaignID(campaignid, hotelid, from_date, to_date);
        ...
        public async Task<IActionResult> SummitHotelPolicy(string model)
        {
            ...
            try
            {
                ...
                //if (summit_model.Detail.ClientTypeId == 2) summit_model.Detail.ClientTypeId = 1;
                status = await _campaignRepository.CreateOrUpdateHotelCampaign(summit_model, userId);
                //_redisService.DeleteCacheByKeyword(CacheName.ClientHotelSearchResult, Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                //_redisService.DeleteCacheByKeyword(CacheName.HotelRoomDetail, Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                //_redisService.DeleteCacheByKeyword(CacheName.HotelCommitB2B, Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                //_redisService.DeleteCacheByKeyword(CacheName.HotelLocationB2B, Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                //_redisService.DeleteCacheByKeyword(CacheName.HotelLocationB2BList, Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                //_redisService.DeleteCacheByKeyword(CacheName.HotelSearchB2B, Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                if (status == (int)ResponseType.SUCCESS)
                {
                    message = "Lưu chính sách giá thành công";
                    var hotel = _hotelRepository.GetHotelById(summit_model.PricePolicy[0].HotelId);
                    await _redisService.DeleteCacheByKeyword(EncodeHelpers.MD5Hash(hotel.HotelId) ,Convert.ToInt32(_configuration["Redis:Database:db_search_result"]));
                }
                else
                {
