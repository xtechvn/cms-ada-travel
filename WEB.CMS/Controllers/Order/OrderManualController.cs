    ...
    public partial class OrderController : Controller
    {
        ...
        public async Task<IActionResult> AddHotelService(long hotel_booking_id)
        {
            try
            {
                ...
                {
                    ViewBag.OrderId = booking.OrderId;
        ...
        public async Task<IActionResult> AddHotelServiceRoom(long hotel_booking_id)
        {
            try
            {
                ...
                ViewBag.OrderId = 0;
                ViewBag.hotel_booking_id = hotel_booking_id;
                    
                var RoomAvailable = _allCodeRepository.GetListByType("BOOKING_HOTEL_ROOM_AVAILABLE");
                ViewBag.RoomAvailable = RoomAvailable;
        ...
        public async Task<IActionResult> UserSuggestion(string txt_search,int service_type=0)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var data_sql = await _userRepository.GetUserSuggesstion(txt_search, list_user_ids.Where(x=> x.Id != null).Select(x=>(int)x.Id ).ToList());
                    if (data_sql != null && data_sql.Count > 0)
                    {
        ...
        public async Task<IActionResult> AddTourService(long order_id, long tour_id)
        {
            ...
            try
            {
                if (order_id > 0)
                {
                    ...
                    ViewBag.User = new UserESViewModel();
                    var user = await _userRepository.GetChiefofDepartmentByServiceType((int)ServiceType.Tour);
                    if (user != null && user.Id>0)
                    {
                        ViewBag.User = new UserESViewModel()
                        {
