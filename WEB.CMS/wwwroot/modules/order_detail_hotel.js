var _order_detail_hotel = {
    ...
    DynamicBindInput: function () {
        ...
        $("body").on('change', ".servicemanual-hotel-room-available", function () {
    ...
    Summit: function () {
        ...
        if ($('#form_add_hotel_service').attr('data-booking-id') != undefined) {
            _msgconfirm.openDialog(_order_detail_html.summit_confirmbox_title, _order_detail_html.summit_confirmbox_create_hotel_service_description, function () {
                ...
                $.ajax({
                    url: "/Order/SummitHotelServiceData",
                    type: "post",
    ...
    CalucateTotalServiceProfit: function () {
        ...
        var Unit = $('#Unit').val()
        var voucher_text = $('#PriceSalesVoucher').val()
        var amount = 0
        var room_amount = package_text == undefined || isNaN(parseFloat(package_text.replaceAll(',', ''))) ? 0 : parseFloat(package_text.replaceAll(',', ''))
        var extra_amount = extra_text == undefined || isNaN(parseFloat(extra_text.replaceAll(',', ''))) ? 0 : parseFloat(extra_text.replaceAll(',', ''))
        var voucher_amount = voucher_text == undefined || isNaN(parseFloat(voucher_text.replaceAll(',', ''))) ? 0 : parseFloat(voucher_text.replaceAll(',', ''))
        amount = room_amount + extra_amount
        ...
        if (parseFloat(Unit) == 0 && voucher_amount != 0) {
            var amount_voucher = 0;
            var sale_price = package_text == undefined || isNaN(parseFloat(package_text.replaceAll(',', ''))) ? 0 : parseFloat(package_text.replaceAll(',', ''))
            $('.servicemanual-hotel-room-tr').each(function (index, item) {
                var element = $(item);
                var sale_price = 0;
                var nights = 0;
                var rooms = element.find('.servicemanual-hotel-room-number-of-rooms').val().replaceAll(',', '');
                var type = element.find('.servicemanual-hotel-room-available').val() ;
                if (type == 1)
                    element.find('.servicemanual-hotel-room-rates-code').each(function (index_2, item_2) {
                        var package_element = $(item_2);
                        var rate_id = package_element.attr('data-rate-id').trim()
                        ...
                        var sale_price_element = _order_detail_hotel.GetHotelRatesAmount(element, rate_id)
                        var nights_element = _order_detail_hotel.GetHotelRatesNights(element, rate_id)

                        sale_price = sale_price_element.val().replaceAll(',', '');
                        nights = parseFloat(nights_element.val().replaceAll(',', ''));

                        amount_voucher += Math.round( parseFloat(sale_price) * voucher_amount / 100 * parseFloat(nights) * parseFloat(rooms));
                    });
            });

            $('#servicemanual-hotel-voucher').val(_global_function.Comma(amount_voucher));
            $('#servicemanual-hotel-voucher').html(_global_function.Comma(amount_voucher));
        } else {
            var amount_voucher = 0;
            ...
            $('.servicemanual-hotel-room-tr').each(function (index, item) {
                ...
                if (type == 1)
                    element.find('.servicemanual-hotel-room-rates-code').each(function (index_2, item_2) {
                        ...
                        var nights_element = _order_detail_hotel.GetHotelRatesNights(element, rate_id)
                       var nights = parseFloat(nights_element.val().replaceAll(',', ''));
                        amount_voucher += voucher_amount * parseFloat(nights) * parseFloat(rooms);
                    });
            });
                ...
                processResults: function (response) {
                    return {
                        results: $.map(response.data, function (item) {
                            return {
                                text: item.name,
                                id: item.hotelid == "0" ? item.id : item.hotelid,
                            }
                        })
                    };
                },
    ...
    HotelServiceRoomPackagesPopup: function (hotel_booking_id) {
        $.ajax({
            url: "/Order/AddHotelServiceRoomPackages",
            type: "post",
            data: { hotel_booking_id: hotel_booking_id },
            success: function (result) {
                if (result != undefined) {
    ...
    HotelServiceRoomGuestPopup: function (hotel_booking_id) {
        $.ajax({
            url: "/Order/AddHotelServiceGuest",
            type: "post",
            data: { hotel_booking_id: hotel_booking_id },
            success: function (result) {
                if (result != undefined) {
